...
class ProductCacheIntegrationTest {
    ...
    void list_evicted_afterCreateProduct() {
        ...
        List<ProductListItemInfo> first = productFacade.getProducts(null, ProductSortType.LATEST, null, 20, null).items();
        int before = first.size();

        createProduct("상품B", 100_000L, 10);                      // evictListForNewProduct

        List<ProductListItemInfo> second = productFacade.getProducts(null, ProductSortType.LATEST, null, 20, null).items();
