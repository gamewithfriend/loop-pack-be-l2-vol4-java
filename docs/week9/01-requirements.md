...
- **FR-6** 상품 상세(`GET /api/v1/products/{id}/detail`)는 해당 상품의 **오늘 순위**(랭킹에 없으면 null)를 포함한다.
...
- **NFR-3 조회 성능** — 상위 N 조회는 `ZREVRANGE`로 O(log N + M). 랭킹 페이지 조립 시 상품/브랜드/좋아요수는 **배치 조회로 N+1 회피**.
- **NFR-4 저장 비용 한계** — 일간 키 TTL 2일로 과거 데이터를 자동 회수한다.
